...
export function Hero() {
  ...
  return (
    <section className="relative -mt-16 min-h-svh w-full overflow-hidden bg-bg md:-mt-20">
      <motion.div
        initial={reduce ? false : { scale: 1.02, opacity: 0, filter: 'brightness(0.96)' }}
      ...
      >
        <img
          ...
          alt=""
          aria-hidden="true"
          className="h-full w-full -translate-y-9 select-none object-cover object-center"
          decoding="async"
          draggable={false}
        />
      </motion.div>
      ...
      <Container className="relative z-10 flex min-h-[inherit] w-full flex-col justify-center py-24">
        <div className="max-w-2xl">
          ...
          <motion.div {...rise(0.15)} className="mt-6">
            <p className="text-base text-ink-inverse-muted md:text-lg">Created by artists.</p>
            <p className="text-base text-ink-inverse-muted md:text-lg">Shared with everyone.</p>
          </motion.div>
          <motion.div {...fade(0.3)} className="mt-8">
            ...
            >
              Step inside
              <RxArrowRight className="size-4" aria-hidden="true" />
            </Link>
          </motion.div>
        </div>
      </Container>
